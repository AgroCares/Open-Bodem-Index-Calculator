...
# plot new_score against rsl
nsdt <- data.table(x = seq(0,1,0.01), y = OBIC::evaluate_logistic(x = seq(0,1,0.01), b = 15, x0 = 0.75, v = 1, increasing = TRUE))
ggn <- ggscatter(nsdt, x = 'x', y = 'y') + theme_pubr(base_size = 10) + ylab('Bewerkbaarheidsscore') + xlab('Relatieve lengte bewerkbare periode')
show(ggn)
ggsave(filename = '../OBIC functies bodembewerkbaarheid/indicator_scores.png',plot = ggn, width = 9.8*1.61803/2,height = 9.8/2, units = 'cm')
