...
In the baseline scenario, total scores are somewhat lower when aggregating logarithmically or linearly compared to using no special aggregation method. In all three methods, the indicator values in the baseline are around `r round(mean(dta[treatment == 'baseline', value]),2)`, this number is preserved in the score when averaging all indicator values (cf_method = non) while log and lin scores are on average `r round(mean(dta[treatment == 'baseline', value]) - dtt[treatment == 'baseline'& cf_method == 'log',S_T_OBI_A],2)` and `r round(mean(dta[treatment == 'baseline', value]) - dtt[treatment == 'baseline'& cf_method == 'lin',S_T_OBI_A],2)` lower. 0.05 may seem like a small number but one has to remember that scores range runs from 0 to 1. So, on a scale of 0 to 10, this would translate to a reduction of 0.5. The cf_method 'non', consistently scores higher then the special aggregation methods, with the exception of scenario 'Recent years high'.
...
# get indicator values per field
for(i in unique(binnenveld$ID)){
...
# make labels
ldt <- binnenveld[,.(ID, YEAR, B_LU_BRP, B_SOILTYPE_AGR, B_GWL_CLASS)]
...
bini[grepl('^S_T_', cat), lcat := "Total"]
bini[grepl('^S_C_', cat), lcat := "Chemical"]
bini[grepl('^S_B_', cat), lcat := "Biological"]
bini[grepl('^S_P_', cat), lcat := "Physical"]
bini[grepl('^S_M_', cat), lcat := "Management"]
bini[grepl('^S_E_', cat), lcat := "Environmental"]
bini[grepl('^S_Tn', cat), lcat := 'Total no cat']
bini[, lcat := factor(lcat, levels = c('Chemical', 'Physical', 'Biological', 'Environmental',
                                       'Management','Total', 'Total no cat'))]


# make plot
gg <- ggplot(bini, aes(x= lcat, y= score, color = cf_method, alpha = 0.5)) +
  geom_point(size = 2) +
  ylab('OBI-score') + xlab('') +
