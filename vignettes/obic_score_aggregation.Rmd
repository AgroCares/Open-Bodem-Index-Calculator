...
Subsequently, the score of each category is computed by aggregating the weighted values of all soil indicators within the category:
$$
S =  \frac{\sum_{i}(vcf_{i} I_{i})}{\sum_{i}vcf_{i}}
$$
...
Aggregation of scores over years is done with the following two lines of code. This is analogue to the aggregation procedure within each category as described above (i.e. sum of weighted score, divided by the sum of all weighing factors).
...
aggscores_brent <- function(dt) {
  ...
  dt.agg <- copy(dt)
  	...
  	# calculate weighted value per category and year
	dt.agg <- dt.agg[,w.value := sum(v_cf* pmax(0,value) / sum(v_cf[value >= 0])), by = .(treatment, field, cf_method, cat, year)]
	
	# calculated weighted average value per category (so mean over years)
	dt.agg <- dt.agg[,wy.value := sum(y_cf * pmax(0, w.value) / sum(y_cf[w.value >= 0])), by = .(treatment, field, cf_method, cat)] # scores per category
	
	# calculate total obi score
	dt.agg <- dt.agg[,S_T := sum(wy.value * c_cf / sum(c_cf)), by = .(treatment, field, cf_method)]
	
	# calculate total obi score if not aggregated by cat
	dt.agg <- dt.agg[, nocat.value := sum(v_cf* pmax(0,value) / sum(v_cf[value >= 0])), by = .(treatment, field, cf_method, year)]
	dt.agg <- dt.agg[, S_T_nocat := sum(y_cf * pmax(0, nocat.value) / sum(y_cf[nocat.value >= 0])), by = .(treatment,  field, cf_method)]
