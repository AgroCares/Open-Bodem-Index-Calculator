...
In the figure below, we calculated the total OBIC score both with and without aggregation per category. 
...
The effects of aggregation steps on total score is large, and it is difficult to trace where and how the total score is influenced by different aggregation steps. Computing category scores before aggregating to a total score is therefore a nice way to show intermediate steps, and allow users to interpret different aspects of soils. 
...
gg3 <- ggplot(dt2[cat %in% c('S_T_cf_log', 'S_T_cf_lin', 'S_T_cf_non', 'S_T_nocat') & 
                  cf_method == 'log' & 
