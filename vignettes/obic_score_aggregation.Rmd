...
# log cf
dt <- dt[,v_cf_log := OBIC::cf_ind_importance(value)]

# lin cf
dt <- dt[,v_cf_lin := 5-4.17*value]
...
# log cf
dt <- dt[,y_cf_log := log(12 - pmin(10,year))]

# lin cf
dt <- dt[,y_cf_lin := 2.59-0.19*value]
...
# log cf
dt <- dt[,c_cf_log := log(ncat + 1)]

# log lin
dt <- dt[,c_cf_lin := ncat*log(10 + 1)/10]
