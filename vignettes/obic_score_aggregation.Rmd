...
Note that the variation of total score is not larger for 'lin' and 'log' than 'non', irrespective of the larger variations in the correction factors for 'lin' and 'log'. To illustrate the pattern, we zoom up to the baseline scenario (Fig 3). Here we split the 'log' method into 4 variations: when 'log' method is applied for all 3 aggregation steps ('log_all'), only for the indicator aggregation ('log_vcf'), year aggregation ('log_ycf'), or category aggregation ('log_ccf'). Same applies to 'lin' method. Standard deviation of total score is slightly smaller when 'log' or 'lin' method is used (SD = `r round(dt3.out[cat == 'total' & treatment =="baseline" & cf_method == "log_all", sd(value)],3)` and , `r round(dt3.out[cat == 'total' & treatment =="baseline" & cf_method == "lin_all", sd(value)],3)`respectively) compared to no special aggregation method is used ('non', SD = `r round(dt3.out[cat == 'total' & treatment =="baseline" & cf_method == "non", sd(value)],3)`). Looking closer at different steps of aggregation, the variation increases due to indicator aggregation (*vcf*) and the year aggregation (*ycf*), but decreased with category aggregation (*ccf*).  As a result, the variation is smaller when linear or log method is applied in all 3 aggregation steps, largely owing to the category aggregation. The reason for the large effect of category aggregation is as follows: Category aggregation increases variation in total score because an extreme indicator value in categories with a few indicators are strongly reflected in the total score and therefore increase the variation, compared to when total scores is directly computed from all indicators. With 'log' and 'lin' methods, both of which gives smaller weights to categories with a few indicators, this impact is diluted. 
