...
dt <- dt[,w.value := sum(v_cf*pmax(0,value) / sum(v_cf[value >= 0])), by = .(field, cat, year, cf_method)]

# calculated weighted average value by year
dt <- dt[,wy.value := sum(y_cf * pmax(0, w.value) / sum(y_cf[value >= 0])), by = .(field, cat, cf_method)]
