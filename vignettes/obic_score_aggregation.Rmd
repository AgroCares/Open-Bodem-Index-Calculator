...
# # (This is a insightful figure, but probably too complicated to show in the vignette.)
...
# 
# ggplot(dt3.out[cat == 'total'], aes(x = value, y = cf_method, fill = method_main, alpha = alpha)) +
#   geom_vline(data = arimean3, aes(xintercept = V1), col = "red", lty = 2) +
#   geom_boxplot() +
#   theme_bw() + scale_colour_viridis_d() + scale_y_discrete(limits = rev) +
#   labs(fill = "")+
...
The reason why 'lin' and 'log' methods yields lower average scores is that low indicator values (i.e. left-hand side in their distribution) are penalized in those methods with the the weighing factor *vcf*. Since the 'lin' and 'log' methods were harmonized for their highest value at low indicator value (cf = 5 for value = 0), the penalty at intermediate indicator values are much higher for 'lin' method. This makes the average value of the OBIC score lower for 'lin' than 'log' methods. The other two aggregation steps (year aggregation (with *ycf* ) and category aggregation (with *ccf*)) causes no difference in the baseline scenario. 

Note that the variation of total score is not larger for 'lin' and 'log' than 'non', irrespective of the larger variations in the correction factors for 'lin' and 'log'. Although the indicator aggregation (with *vcf*) and the year aggregation (with *ycf*) slightly increase the variation in total score, the category aggregation (with *ccf*) reduces the variation, resulting in similar variation among the three methods. Furthermore, the variation caused by *vcf* is limited because the correction factor *vcf* and indicator value are inversely linked, and therefore the SD of 'indicator value times correction factors' do not differ much among the three methods.
