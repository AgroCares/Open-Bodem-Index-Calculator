...
dt <- dt[,w.value := sum(v_cf*pmax(0,value) / sum(v_cf[value >= 0])), by = .(field, cf_method, indicator, year)]

# calculated weighted average value by year
dt <- dt[,wy.value := sum(y_cf * pmax(0, w.value) / sum(y_cf[value >= 0])), by = .(field, cf_method, cat)] # scores per category
...
dts <- unique(dt[,.(field, indicator,cat, wy.value, S_T ,treatment, cf_method)])
...
# bind scores dt's
dt <- rbindlist(list(dts1, dts2), use.names = TRUE)
...
dt <- dt[, cat := factor(cat, levels = c('S_T_OBI_A', 'S_C_OBI_A', 'S_P_OBI_A', 'S_B_OBI_A', 'S_E_OBI_A'))]
