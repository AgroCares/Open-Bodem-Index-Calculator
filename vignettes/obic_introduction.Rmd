...
```{r,fig.width = 7, fig.height = 4,fig.fullwidth = TRUE,echo=FALSE}
  ...
  dd <- data.table(D_NLV = S,
    ...
    # Calculate indicators for soil biological functions
    dd[, I_B_DI := ind_resistance(A_SOM_LOI)]
    ...
    # ggplot2 way
    ggs <- ggplot(data = dd.melt, aes(x= function_value, y = index_value, group = B_LU_BRP, 
                                      facet_wrap(facets = indicator, ncol = 4))) +
      geom_point() + theme_bw() + scale_fill_viridis_d()+ scale_color_viridis_d()

    # ggpubr way
    ggs <- ggscatter(dd.melt, x = 'function_value', y = 'index_value', color = 'B_LU_BRP',
