...
Note that `binnenveld` is a data.table, a modern and fast variant of a data frame [@R-data.table]. It's particularly useful for large datasets because it only prints a selection of the dataset and calculations done on a data.table are extremely fast. You can learn more about data.tables at [this link](https://cran.r-project.org/web/packages/data.table/vignettes/datatable-intro.html) ; in particular you can convert data frames to data.tables with as.data.table().
...
```{r,fig.width = 7, fig.height = 4,fig.fullwidth = TRUE,echo=FALSE}
  ...
  S <- rep(seq(0,100,1),2)
  dd <- data.table(D_NLV = S,
                   D_PBI = S,
                   D_K = S,
                   D_MG = S,
                   D_SLV = S,
                   D_PH_DELTA = rep(seq(0,5,0.05),2),
                   D_CEC = S,
                   D_CU = S,
                   D_ZN = S,
                   D_CR = rep(seq(0,10,0.1),2),
                   D_SE = rep(seq(0,50,0.5),2),
                   D_WSI_DS = S,
                   D_WSI_WS = S,
                   D_WE = rep(seq(0,1,0.01)),
                   D_WRI = S,
                   D_AS = rep(seq(0,1,0.01)),
                   D_PMN = S,
                   D_NGW = S,
                   D_NSW = S,
                   B_LU_BRP = c(rep(265,101),rep(256,101)),
                   B_SOILTYPE_AGR = rep('dekzand',202),
                   A_SOM_LOI = rep(3.5,202),
                   D_DUMMY_SOM_LOI = rep(seq(0,35,0.35),2),
                   B_AER_CBS = rep("Centraal Veehouderijgebied",202)
                   )
    dd <- dd[,id:= 1:nrow(dd)]
  
     # Calculate indicators for soil chemical functions
    dd[, I_C_N := ind_nitrogen(D_NLV, B_LU_BRP)]
    dd[, I_C_P := ind_phosphate_availability(D_PBI)]
    dd[, I_C_K := ind_potassium(D_K,B_LU_BRP,B_SOILTYPE_AGR,A_SOM_LOI)]
    dd[, I_C_MG := ind_magnesium(D_MG, B_LU_BRP, B_SOILTYPE_AGR)]
    dd[, I_C_S := ind_sulpher(D_SLV, B_LU_BRP, B_SOILTYPE_AGR, B_AER_CBS)]
    dd[, I_C_PH := ind_ph(D_PH_DELTA)]
    dd[, I_C_CEC := ind_cec(D_CEC)]
    dd[, I_C_CU := ind_copper(D_CU,B_LU_BRP)]
    dd[, I_C_ZN := ind_zinc(D_ZN)]
    
    # Calculate indicators for soil physical functions
    dd[, I_P_CR := ind_crumbleability(D_CR, B_LU_BRP)]
    dd[, I_P_SE := ind_sealing(D_SE, B_LU_BRP)]
    dd[, I_P_DS := ind_waterstressindex(D_WSI_DS)]
    dd[, I_P_WS := ind_waterstressindex(D_WSI_WS)]
    dd[, I_P_DU := ind_winderodibility(D_WE)]
    # dd[, I_P_CO := ind_compaction(B_SC_WENR)] # is categorical
    dd[, I_P_WRI := ind_waterretention(D_WRI)]
    dd[, I_P_CEC := ind_aggregatestability(D_AS)]
    #dd[, I_P_WO := ind_workability(D_WO)] (WIP)
  
    # Calculate indicators for soil biological functions
    dd[, I_B_DI := ind_resistance(D_DUMMY_SOM_LOI)]
    dd[, I_B_SF := ind_pmn(D_PMN)]
    
      # Calculate indicators for environment
    dd[, I_E_NGW := ind_nretention(D_NGW, leaching_to = "gw")]
    dd[, I_E_NSW := ind_nretention(D_NSW, leaching_to = "ow")]
    
    indcols <- colnames(dd)[grep('^I_',colnames(dd))]
    cols <- colnames(dd)[grep('^I_|^B_|^A_|^id$',colnames(dd))]
    dd.melt <- melt.data.table(dd, id.vars = cols,variable.name = 'funct', value.name = 'function_value')
    ...
    dd.melt.select <- dd.melt[(funct == 'D_NLV'& indicator == 'I_C_N')|
                                ...
                                (funct == 'D_CEC'& indicator == 'I_P_AS')|
                                (funct == 'D_PMN'& indicator == 'I_B_SF')|
                                (funct == 'D_NGW'& indicator == 'I_E_NGW')|
                                (funct == 'D_NSW'& indicator == 'I_E_NSW')]
    
    # select indicators for which function values do not run from 0 to 100
    dd.melt.select.other <- dd.melt[(funct == 'D_CR'& indicator == 'I_P_CR')|
                                      (funct == 'D_SE'& indicator == 'I_P_SE')|
                                      (funct == 'D_WE'& indicator == 'I_P_DU')|
                                      (funct == 'D_DUMMY_SOM_LOI'& indicator == 'I_B_DI')]
    
    # ggpubr way
    ggs <- ggscatter(dd.melt.select, x = 'function_value', y = 'index_value', color = 'B_LU_BRP',
                     facet.by = 'indicator') + theme_bw()+ scale_fill_viridis_d()+ scale_color_viridis_d()

    # make seperate graphs for indicators whose function values do not run from 0 to 100
    PCR <- ggscatter(dd.melt.select.other[indicator == 'I_P_CR'], x = 'function_value', y = 'index_value', color = 'B_LU_BRP') + 
      theme_bw()+ scale_fill_viridis_d()+ scale_color_viridis_d()
    PSE <- ggscatter(dd.melt.select.other[indicator == 'I_P_SE'], x = 'function_value', y = 'index_value', color = 'B_LU_BRP') + 
      theme_bw()+ scale_fill_viridis_d()+ scale_color_viridis_d()
    PDU <- ggscatter(dd.melt.select.other[indicator == 'I_P_DU'], x = 'function_value', y = 'index_value', color = 'B_LU_BRP') + 
      theme_bw()+ scale_fill_viridis_d()+ scale_color_viridis_d()
    BDI <- ggscatter(dd.melt.select.other[indicator == 'I_B_DI'], x = 'function_value', y = 'index_value', color = 'B_LU_BRP') + 
      theme_bw()+ scale_fill_viridis_d()+ scale_color_viridis_d()
    # arrange these graphs with common legend
    ggs2 <- ggarrange(PCR, PSE, PDU, BDI, ncol = 2, nrow = 2, common.legend = TRUE)
