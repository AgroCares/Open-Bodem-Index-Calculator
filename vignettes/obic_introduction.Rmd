...
```{r,fig.width = 7, fig.height = 4,fig.fullwidth = TRUE,echo=FALSE}
  ...
  # add all derivatives
  dt[,D_PH_DELTA := rep(seq(0,5,0.05),2)]
  dt[,c('D_NLV','D_PBI','D_K','D_MG','D_SLV','D_CEC','D_CU','D_ZN','D_PMN') := rep(seq(0,100,1),2)]
  dt[,c('D_WSI_DS','D_WSI_WS','D_WRI','D_NGW','D_NSW') := rep(seq(0,100,1),2)]
  ...
  dt[, I_P_WS := ind_waterstressindex(D_WSI_WS)]
  dt[, I_P_DU := ind_winderodibility(D_WE)]
  # dt[, I_P_CO := ind_compaction(B_SC_WENR)] # is categorical
  dt[, I_P_WRI := ind_waterretention(D_WRI)]
  dt[, I_P_CEC := ind_aggregatestability(D_AS)]
  #dt[, I_P_WO := ind_workability(D_WO)] (WIP)
