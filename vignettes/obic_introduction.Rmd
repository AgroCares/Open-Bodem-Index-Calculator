...
Below, examples are give of how to calculate the organic matter balance and the management index.
...
Example of the soil organic matter balance on five fields in 2010 from the Binnenveld dataset

```{r,fig.width = 7, fig.height = 4,fig.fullwidth = TRUE,echo=FALSE}
    dt <- binnenveld[ID %in% head(unique(binnenveld$ID),5)&YEAR == 2010]
    dt[, D_SOM_BAL := calc_sombalance(B_LU_BRP,A_SOM_LOI, A_P_AL, A_P_WA, M_COMPOST, M_GREEN)]
    
    p2 <- ggplot(data = dt,aes(y = D_SOM_BAL, x = ID, fill = ID)) + geom_col() +  theme_bw() + 
        ylab('SOM balnce') + xlab('Field') + 
        theme(legend.position = c('none'),
              plot.title = element_text(size=10),
              legend.text = element_text(size=10),
