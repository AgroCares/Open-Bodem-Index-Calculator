...
```{r,fig.width = 7, fig.height = 4,fig.fullwidth = TRUE,echo=FALSE}
  ...
  dt.test2 <- dt.test[ID==1][,ID := .I]
  dt.test2 <- dt.test2[rep(ID,4)][,B_GWL_CLASS := rep(c('GtIII','GtIV','GtV','GtVI'),each = max(ID))]
...
```{r,fig.width = 7, fig.height = 4,fig.fullwidth = TRUE,echo=FALSE}
    ...
    p1 <- ggplot(data = dt2[grepl('CEC',variable)],
          ...
          ylab('Soil compaction') + xlab('Fieldscore VSA')+
          theme(legend.position = 'none',
                plot.title = element_text(size=8),
                legend.text = element_text(size=10),
                axis.text = element_text(size=10)) + 
          ggtitle('Soil compaction before and after correction with VSA data\noriginally evaluated as a soil with low compaction')
    
    p2 <- ggplot(data = dt2[grepl('CO',variable)],
          aes(y = value, x = id-1, fill = treatment)) + 
          geom_bar(stat='identity',position=position_dodge())+
          theme_bw() + scale_fill_viridis_d()+ 
          ylab('Aggregate stability') + xlab('Fieldscore VSA')+
          theme(legend.position = c(0.2,0.8),
                plot.title = element_text(size=8),
                axis.text = element_text(size=10)) + 
          ggtitle('Aggregate stability before and after correction withVSA data\noriginally evaluated as a soil with a moderate aggregate stability')
