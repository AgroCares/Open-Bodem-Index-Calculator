...
```{r,fig.width = 7, fig.height = 4,fig.fullwidth = TRUE,echo=FALSE, eval=FALSE}
   ...
   p1 <- ggplot(data = dt,aes(y = D_CEC, x = A_CEC_CO)) + 
        geom_point(show.legend = FALSE) + geom_line()+ theme_bw() + 
...
```{r,fig.width = 7, fig.height = 4,fig.fullwidth = TRUE,echo=FALSE}
    ...
    p1 <- ggplot(data = dt,aes(y = I_P_CO, x = id, fill = id)) + geom_col() +  theme_bw() + 
    ...
    p2 <- ggplot(data = dt,aes(y = I_P_CEC, x = id, fill = id)) + geom_col() +  theme_bw() + 
...
```{r,fig.width = 7, fig.height = 4,fig.fullwidth = TRUE,echo=FALSE}
    ...
    # make a plot
    p1 <- ggplot(data = dt,
               aes(y = I_B_DI, x = A_SOM_LOI)) + geom_line() + geom_point(size=3) +  theme_bw() + 
        theme(plot.title = element_text(size=10),
                  legend.text = element_text(size=10),
                  axis.text = element_text(size=10),
              legend.position = c(0.75,0.15))+
...
```{r,fig.width = 7, fig.height = 4,fig.fullwidth = TRUE,echo=FALSE}
  ...
  p <- ggplot(data = dt.test, aes(y = I_NGW, x = ngw)) + geom_line()  +
    theme_bw() + xlab('Potential N leaching')
...
```{r,fig.width = 7, fig.height = 4,fig.fullwidth = TRUE,echo=FALSE}
    ...
    p2 <- ggplot(data = dt,aes(y = D_SOM_BAL, x = ID, fill = ID)) + geom_col() +  theme_bw() + 
    ...
    p3 <- ggplot(data = dt,aes(y = D_MAN, x = ID, fill = ID)) + geom_col() +  theme_bw() + 
...
```{r,fig.width = 7, fig.height = 4,fig.fullwidth = TRUE,echo=FALSE}
  ...
  p.ar <- ggplot(data = dt.arabl,
                 aes(points,score, col = landuse, group = landuse, shape = landuse)) + geom_point(size = 3)+ geom_line() + theme_bw() +
...
bar[grepl('^S_E_', variable), cat := "Environmental"]
   ...
   g2 <- ggdotchart(
      ...
      group = "cat", color = "cat", palette = c("#33CCFF",'#666666', "#513000", "#3B6B20", "#FF6600", "#7CC144" ),
      # group = "cat", color = "cat", palette = c("#3333CC","#33CCFF", "#513000", "#3B6B20", "#FF6600", "#7CC144"), # c('#7CC144', '#FF6600','#3B6B20', '#513000', '#33CCFF', "#3333CC", "#5A5A5A"), # orgineel palette "jco"
      add = "segment", size=3,
