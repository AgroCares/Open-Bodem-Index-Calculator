...
```{r,fig.width = 7, fig.height = 4,fig.fullwidth = TRUE,echo=FALSE}
  ...
  dd <- data.table(D_NLV = S,
    ...
    # ggplot2 + patchwork
    ggsp <- ggplot(dd.melt.select, aes(x= function_value, y= index_value)) +
      geom_line(mapping = aes(color = B_LU_BRP), size = 1.5) + facet_wrap(facets = 'indicator', ncol = 4) + theme_bw() + 
      scale_fill_viridis_d()+ scale_color_viridis_d()
    
    # ggplot2 + patchwork
    PCRp <- ggplot(dd.melt.select.other[indicator == 'I_P_CR'], aes(x= function_value, y= index_value, color = B_LU_BRP)) +
      geom_line(size = 1.5) + theme_bw() + scale_fill_viridis_d()+ scale_color_viridis_d()
    
    PSEp <- ggplot(dd.melt.select.other[indicator == 'I_P_SE'], aes(x= function_value, y= index_value, color = B_LU_BRP)) +
      geom_line(size = 1.5) + theme_bw() + scale_fill_viridis_d()+ scale_color_viridis_d()
    
    PDUp <- ggplot(dd.melt.select.other[indicator == 'I_P_DU'], aes(x= function_value, y= index_value, color = B_LU_BRP)) +
      geom_line(size = 1.5) + theme_bw() + scale_fill_viridis_d()+ scale_color_viridis_d()
      
    BDIp <- ggplot(dd.melt.select.other[indicator == 'I_B_DI'], aes(x= function_value, y= index_value, color = B_LU_BRP)) +
      geom_line(size = 1.5) + theme_bw() + scale_fill_viridis_d()+ scale_color_viridis_d()
    
    # patch plots
    (ggsp)/(PCRp + PSEp + PDUp + BDIp) + plot_layout(guides = 'collect') + 
      theme(plot.caption = element_text(hjust = 0))+
            labs(caption = 'Relation between function values and their corresponding evaluated index values.')
