...
---
nocite: |
  @Rinot2019, @Bunemann2018, @R-base, @R-data.table, @R-ggplot2, @R-ggpubr, @R-OBIC, @R-patchwork
---
...
In order to improve and maintain soil quality while supporting sustainable agronomic production as well as other soil ecosystems services, the Open Soil Index was initiated in 2018, an open source framework for assessing and valorising soil quality and sustainable soil management. Experts from universities, consultancy companies, soil experts and farmer groups collaborated to bring together existing data from routine soil laboratories, smart data retrieval from satellites, insights from fundamental and applied research in a transparent and scientifically underpinned soil assessment framework. The framework is highly influenced by the Soil Management Assessment Framework [@Andrews2001;@Wienhold2004;@Wienhold2009] and the proposed holistic, multivariate soil health framework from Rinot et al. (2019). 
...
### Magnesium supply
Magnesium is an important element for crop growth and livestock health and productivity.
...
```{r,fig.width = 7, fig.height = 4,fig.fullwidth = TRUE,echo=FALSE}
  ...
  p1 <- ggplot(data = dt.test1,aes(y = value, x = ID, fill = stresstype)) + 
        geom_bar(stat='identity') +  theme_bw() + scale_fill_viridis_d()+ scale_color_viridis_d()+ ylim(0,50)+
  ...
  p2 <- ggplot(data = dt.test2,aes(y = value, x = B_GWL_CLASS, fill = stresstype)) + 
        geom_bar(stat='identity') +  theme_bw() + scale_fill_viridis_d()+ scale_color_viridis_d()+ ylim(0,30)+
        theme(legend.position = c(0.3,0.8),
              plot.title = element_text(size=10),
              legend.text = element_text(size=10)) +
...
```{r,fig.width = 7, fig.height = 4,fig.fullwidth = TRUE,echo=FALSE}
  ...
  # plot output aggregate stability
    p4 <- ggplot(data = dt4,
               aes(x = A_CA_CO_PO, y = D_AS, color = B_SOILTYPE_AGR)) + 
        geom_point(size=3) +  theme_bw() + scale_fill_viridis_d()+ scale_color_viridis_d()+ 
        ylab('Aggregate stability') + xlab('Calcium ocupation') + ylim(0,1)+
        theme(plot.title = element_text(size=10),
              legend.position = 'none') +
    ggtitle('Aggregate stability with given Ca occupation and\nrandom Mg and K occupation')
...
obic_field wraps a function to format the information from the soil compaction risk assessment map. This is required because the risk assessment map data can be numeric or string. The format_soilcompaction function checks the inputs and converts them to the correct strings when the inputs are okay. The possible risks for compaction a parcel can have are: "zeer beperkt" (very limited), "beperkt" (limited), "matig" (average), "groot" (large) and "zeer groot" (very large). Other possible values which are not typically found on agricultural parcels are: "bebouw" (built up), "water" and "glastuinbouw" (greenhouse horticulture) 
...
Within the OBIC, the estimates of aggregate stability and risk of subsoil compaction can be adapted to local site specific insights derived from the Visual Soil Assessment Methodology. Visual soil assessments (VSA) asses soil quality in the field, by digging a hole and assess several soil quality indicators visually. Soil quality indicators are: grass cover, number of roots, number of biopores, soil colour, soil structure, soil compaction and number of gray spots. Visual soil assessments are more and more used by farmers in the Netherlands, but not specifically developed for various soil types. 

Below is illustrated how the obic_field wrapper deals with the parameters derived from the VSA methodology. The score derived from the origignal risk assessment map for subsoil compaction is overwritten with the relevant VSA indicators when When scores are present for the presence of earthworms (A_EW_BCS), the occurrence of subsurface compaction (A_SC_BCS), limited root development (A_RD_BCS), ponding in the spring (wet conditions after rainfall, A_P_BCS) and clear presence of visible tracks / rutting or trampling on the land (A_RT_BCS). Similarly, the soil aggregate stability is adapted when VSA indicators are available for earthworm density (A_EW_BCS), soil structure (A_SS_BCS) and presence of cracks in the topsoil (A_C_BCS).
...
```{r,fig.width = 7, fig.height = 4,fig.fullwidth = TRUE,echo=FALSE}
    ...
    pclay <- ggplot(data = dt[plot == 'klei'],
               aes(x = A_CLAY_MI, y = paw)) + geom_line() + geom_point(size=3) +  theme_bw() + scale_fill_viridis_d()+ scale_color_viridis_d()+ 
        theme(plot.title = element_text(size=10),
                  legend.text = element_text(size=10),
                  axis.text = element_text(size=10))+
        ylab('Plant available water') + xlab('Clay content (%)') +
        ggtitle('Relation between plant water \navailability and clay content') 

    pcsand <- ggplot(data = dt[plot == 'zand'],
               aes(x = A_SAND_MI, y = paw)) + geom_line() + geom_point(size=3) +  theme_bw() + scale_fill_viridis_d()+ scale_color_viridis_d()+ 
        theme(plot.title = element_text(size=10),
                  legend.text = element_text(size=10),
                  axis.text = element_text(size=10))+
        ylab('Plant available water') + xlab('Sand content (%)') +
        ggtitle('Relation between plant water \navailability and sand content')
    
    pcsilt <- ggplot(data = dt[plot == 'silt'],
               aes(x = A_SILT_MI, y = paw)) + geom_line() + geom_point(size=3) +  theme_bw() + scale_fill_viridis_d()+ scale_color_viridis_d()+ 
        theme(plot.title = element_text(size=10),
                  legend.text = element_text(size=10),
                  axis.text = element_text(size=10))+
        ylab('Plant available water') + xlab('Silt content (%)') +
        ggtitle('Relation between plant water \navailability and silt content')
    
    pcsom <- ggplot(data = dt[plot == 'som'],
               aes(x = A_SOM_LOI, y = paw)) + geom_line() + geom_point(size=3) +  theme_bw() + scale_fill_viridis_d()+ scale_color_viridis_d()+ 
        theme(plot.title = element_text(size=10),
                  legend.text = element_text(size=10),
                  axis.text = element_text(size=10))+
        ylab('Plant available water') + xlab('Soil organic matter content (%)') +
        ggtitle('Relation between plant water \navailability and SOM content')
    
    p1 <- ggarrange(pclay, pcsand, pcsilt, pcsom, ncol = 2, nrow = 2)
    p1
...
```{r,fig.width = 7, fig.height = 4,fig.fullwidth = TRUE,echo=FALSE}
  ...
  # plot output
  p1 <- ggplot(data = dt.test,
               aes(y = D_PMN, x = A_N_PMN, group = landuse, colour = landuse)) + geom_line() + geom_point(size=3) +  theme_bw() + scale_fill_viridis_d()+ scale_color_viridis_d()+ 
        theme(plot.title = element_text(size=10),
                  legend.text = element_text(size=10),
                  axis.text = element_text(size=10),
  ...
  # Plot indicator
    p2 <- ggplot(data = dt.test,
               aes(y = I_PMN2, x = A_N_PMN2, group = landuse, colour = landuse)) + geom_line() + geom_point(size=3) +  theme_bw() + scale_fill_viridis_d()+ scale_color_viridis_d()+ 
        theme(plot.title = element_text(size=10),
                  legend.text = element_text(size=10),
                  axis.text = element_text(size=10),
              legend.position = c(0.75,0.15))+
        ylab('PMN indicator score') + xlab('Microbial activity') +
...
### Disease resistance
The activity of the soil life is an indication of the general disease resistance of the soil. When a soil food web is in balance, the soil's resistance to disease and pests is also optimal. The soil's resistance to disease and pests is often cited as a characteristic of a soil with good biological soil quality. The idea behind this is that a soil with a well-developed soil food web will be in balance, which means that strong multiplication of parasitic soil organisms is inhibited by  other soil life. Mechanisms include competition for food or infection sites on the root, eating the pathogens before they have had a chance to multiply (myco-parasitism), or secreting (specific) substances that pathogens cannot tolerate (antibiosis or soil fungistasis). 
...
```{r,fig.width = 7, fig.height = 4,fig.fullwidth = TRUE,echo=FALSE}
    ...
    # make a plot
    p1 <- ggplot(data = dt,
               aes(y = I_B_DI, x = A_SOM_LOI)) + geom_line() + geom_point(size=3) +  theme_bw() + scale_fill_viridis_d()+ scale_color_viridis_d()+ 
        theme(plot.title = element_text(size=10),
                  legend.text = element_text(size=10),
                  axis.text = element_text(size=10),
              legend.position = c(0.75,0.15))+
        ylab('Disease resistance indicator score') + xlab('Soil organic matter content (%)') +
        ggtitle('Relationship between soil organic matter\nand disease resistance indicator score') 
    p1
```
...
An indicator has been developed to assess to what degree plant parasitic nematode infections of a field can be limiting for crop production. 
```{r, results = FALSE, eval = FALSE}
  # The indicator can be calculated by passing a crop code and the number of individuals of a nematode species/genus per 100g soil to ind_nematodes()
  I_NEMA <- ind_nematodes(B_LU_BRP,
...
The capacity of the soil to hold or move nitrogen in the form of nitrate (NO3^-) so that leaching of nitrate to groundwater is prevented is determined by the landuse, soiltype, groundwatertable, N application, and nitrogen supply capacity. Nitrate is either taken up by a crop, denitrified (under anaerobic conditions), leached or washed away (runoff).
The potential for N leaching to groundwater for a soil can be calculated with calc_nleach(). For more details, see ?calc_nleach.
...
Example of this function are given for five parcels with different attributes, as well as for field 1 with different groundwatertables.
The properties of fields one to five are as follows: 

* soiltype (B_SOILTYPE_AGR): rivierklei, rivierklei, moerige_klei, dekzand, dekzand
* crop_code/crop_name (B_LU_BRP): 229 (trees), 233 (winter wheat), 265 (permanent grassland), 259 (silage maize), 265
* groundwaterclass (B_GWL_CLASS): GtVII, GtVI, GtII, GtIII, GtVI
* nitrogen producing capacity (D_NLV): 103.2, 116.8, 250.0, 24.1, 122.6
* agricultural area (B_AER_CBS): Rivierengebied, Rivierengebied, Centraal Veehouderijgebied, Centraal Veehouderijgebied, Riverierengebied

```{r,fig.width = 7, fig.height = 4,fig.fullwidth = TRUE,echo=FALSE}

  # subset a serie of agricultural fields
  dt.test <- binnenveld[ID %in% c(1,2,3,4,5)& YEAR == 2010]
  ...
  dt.test[, D_NGW := calc_nleach(B_SOILTYPE_AGR, B_LU_BRP, B_GWL_CLASS, D_NLV, B_AER_CBS, leaching_to = "gw")]
  
    # estimate potential for N runoff
  dt.test[, D_NSW := calc_nleach(B_SOILTYPE_AGR, B_LU_BRP, B_GWL_CLASS, D_NLV, B_AER_CBS, leaching_to = "ow")]
  
   # plot output
  p1 <- ggplot(data = dt.test, aes(y = D_NGW, x = ID, col = )) + 
        geom_col() +  theme_bw()+ xlab('Field') + ggtitle('Potential for N leaching')
  p2 <- ggplot(data = dt.test, aes(y = D_NSW, x = ID, col = )) + 
        geom_col() +  theme_bw()+ xlab('Field') + ggtitle('Potential for N runoff')
  p3 <- ggarrange(p1, p2, ncol = 2)
  
  
  # Make example with different gwt's
  dt.test2 <- rbindlist(list(OBIC::binnenveld[1],OBIC::binnenveld[1],OBIC::binnenveld[1],OBIC::binnenveld[1],OBIC::binnenveld[1]))
  ...
  p4 <- ggplot(data = dt.test2, aes(y = D_NGW, x = B_GWL_CLASS)) + 
        geom_col() +  theme_bw()+ xlab('B_GWL_CLASS') + ggtitle('Potential for N leaching')
  p5 <- ggplot(data = dt.test2, aes(y = D_NSW, x = B_GWL_CLASS)) + 
        geom_col() +  theme_bw()+ xlab('B_GWL_CLASS') + ggtitle('Potential for N runoff')
  p6 <- ggarrange(p4, p5, ncol = 2)
    
  
  
  # outp <- ggarrange(p1, p4, tab, ncol = 2, nrow = 2, align = 'v')
  p1+p4
```

```{r,fig.width = 7, fig.height = 2.2,fig.fullwidth = TRUE,echo=FALSE, eval = FALSE}
# Table of field properties
  dt.table <- copy(dt.test)
  setnames(dt.table, 'ID', 'Field')
  dt.table <- dt.table[,D_NLV := round(D_NLV, 1)]

  tab <- ggtexttable(dt.table[,.(Field,B_SOILTYPE_AGR, B_LU_BRP, B_GWL_CLASS, D_NLV, B_AER_CBS)],rows = NULL) 
  tab <- tab_add_title(tab,'Field properties to calculate N leaching and runoff')
  tab
```
...
While the same input data is required to calculate leaching and runoff, internally the calculations use different values. Therefore, runoff and leaching may have different values for a parcel as can be seen in the example below where runoff is calculated for the same fields as was done for leaching.
...
Below, examples are give of how to calculate the organic matter balance and the management index. The management index requires inputs that are not obligatory for the wrapper function obic_field. The function add_management will check the presence of management parameter values and give defaults based on soiltype and crop when values are missing. Furthermore, information on the crop rotation and on the age of grassland are calculated by cacl_rotation_fraction and calc_grass_age respectively.
...
```{r,fig.width = 7, fig.height = 4,fig.fullwidth = TRUE,echo=FALSE}
    ...
    p2 <- ggplot(data = dt,aes(y = D_SOM_BAL, x = ID, fill = ID)) + geom_col() +  theme_bw() + scale_fill_viridis_d()+ scale_color_viridis_d()+ 
        ylab('SOM balance') + xlab('Field') + 
        theme(legend.position = c('none'),
              plot.title = element_text(size=10),
              legend.text = element_text(size=10),
              axis.text = element_text(size=10)) + ggtitle('Soil organic matter balance for five fields')
    ...
    p3 <- ggplot(data = dt,aes(y = D_MAN, x = ID, fill = ID)) + geom_col() +  theme_bw() + scale_fill_viridis_d()+ scale_color_viridis_d()+ 
        ylab('Management score') + xlab('Field') + 
        theme(legend.position = c('none'),
              plot.title = element_text(size=10),
              legend.text = element_text(size=10),
              axis.text = element_text(size=10)) + ggtitle('Management')
...
The Open Soil Index is a score number ranging between 0 to 1, that holistically represents the soil quality as well as its management. The score is field specific, accounting for the soil type, geohydrological site properties and the crop rotation sequence of the last decade. The final score can be decomposed to sub scores for different categories reflecting major soil functions (chemical, physical, biological, and management), and these sub scores can further be decomposed to soil function levels. For each soil function, an indicator value ranging between 0 to 1 reflects the current situation given its deviation from a target condition of that specific soil function. With this hierarchical structure, the OBI offers both integral assessment (i.e. whether a soil is overall good or not) and specific assessment (i.e. which soil functions are poorly evaluated and to what extent these soil functions need to be adapted to improve the overall soil quality to the “optimum situation”). 


### Transforming indicator values
Many of the indicator values calculated in the OBIC have a unit and a target value. D_NLV for example is the nitrogen producing capacity of the soil and should have a value between -30 and 250. However,Values calculated for individual indicators of chemical, physical, biological and environmental soil functions are not always meaningful to farmers. Nor is it easy to assess whether a given value is good or whether it requires improvement. Therefore, indicator scores for the soil functions are transformed to a value between 0 and 1, 0 being very poor and 1 being excellent (optimal for agricultural production). This way, the target value is clear and its easy to see whether an indicator on a parcel warrants a lot of improvement or not. Additionally, such scores can easily be turned into scores ranging from 0-10 which is a commonly used scale for grading in the Netherlands, and therefore easy to interpret by Dutch users.

```{r, results= TRUE, eval= FALSE}
include_graphics('vignettes/OBIC_score_integratie.png')
```
The OBIC package has functions that transform the value of a soil function into a score. Examples of such functions are `ind_nitrogen()`, `ind_sealing()`, `ind_pmn()` and `ind_nretention()`. Some of these functions only require the soil function's value. Others may need soil type or the cultivated crop to as these parameters may require different evaluation for specific soil functions.
...
```{r, results = FALSE, eval = FALSE}
   # Calculate indicators for soil chemical functions
    ...
    #dt[, I_P_WO := ind_workability(D_WO)] (WIP)
...
Each soil indicator belongs to one of the major soil functions (chemical, physical, biological, environmental or management). For each function, a score is calculated by aggregating the scores of their underlying soil indicators, with each indicator being assigned a specific weight. The weight of a soil function depends on its value; functions with low scores gain a higher weight. In this way, a poorly performing soil function contributes more to determining an indicators value than an excellently performing function.

### Holistic OBI score
The integration of different soil functions into one score and their relevance in achieving a specific target (like crop yield) is often ignored in soil quality assessments (i.e., each function contributes equally), is based on expert-judgement or derived from multi-variate statistics analysing linkages among soil functions and soil and field properties within large datasets. Large databases are needed in which all relevant soil properties and functions have been determined as well as (historical) management. Unfortunately, these databases are scarce so that expert driven weighing procedures are introduced. Currently, the total OBIC score is a weighted average of the major soil functions where the weight of each major soil function is determined by the number of indicators underlying the major soil function.
...
In order to calculate the total OBI score `S_T_OBI_A`, an integral assessment of soil quality for the following functions is made: soil chemistry `S_C_OBI_A`, soil structure `S_P_OBI_A`, soil biology `S_B_OBI_A` as well as soil management `S_M_OBI_A`.
...
bar[grepl('^S_E_', variable), cat := "Environmental"]
   ...
   g2 <- ggdotchart(
      bar, x = "cat", y = "value", 
      # order = c("Totaal", "Milieu", "Beheer", "Biologie", "Structuur", "Chemie"),
      order = c("Total", 'Management',"Environmental", "Biological", "Physical", "Chemical"),
      group = "cat", color = "cat",
      add = "segment", size=4.5,
      add.params = list(color = "gray", size = 0.75),
      position = position_dodge(0.5),
      facet.by = "field",
      rotate = TRUE, legend = "none",
      ylab ='OBI-score',xlab='', panel.labs = list(field = c('Field 1','Field 2','Field 3', 'Field 4'))
) + scale_fill_viridis_d()+ scale_color_viridis_d()+ theme_bw(base_size = 12) + theme(legend.position = 'none')
