...
```{r plot relation function and index values,fig.width = 7, fig.height = 16,fig.fullwidth = TRUE,echo=FALSE}
  ...
  dd <- data.table(D_NLV = S,
                   D_PBI = rep(seq(0,10,0.1),2),
                   D_K = rep(seq(0,50,0.5),2),
                   D_MG = S,
                   D_SLV = S,
                   D_PH_DELTA = rep(seq(0,5,0.05),2),
                   D_CEC = S,
                   D_CU = rep(seq(0,10,0.1),2),
                   D_ZN = S,
                   D_CR = rep(seq(0,10,0.1),2),
                   D_SE = rep(seq(0,25,0.25),2),
                   D_WSI_DS = rep(seq(0,50,0.5),2),
                   D_WSI_WS = rep(seq(0,50,0.5),2),
                   D_WE = rep(seq(0,1,0.01)),
                   D_WRI = S,
                   D_AS = rep(seq(0,1,0.01)),
                   D_PMN = S,
                   D_NGW = rep(seq(0,25,0.25),2),
                   D_NSW = rep(seq(0,25,0.25),2),
                   B_LU_BRP = c(rep(265,101),rep(256,101)),
                   B_SOILTYPE_AGR = rep('dekzand',202),
                   A_SOM_LOI = rep(3.5,202),
                   D_DUMMY_SOM_LOI = rep(seq(0,10,0.1),2),
    ...
    # make plot
    ggsp <- ggplot(dd.melt, aes(x= function_value, y= index_value)) +
     geom_line(mapping = aes(color = B_LU_BRP, alpha = 0.5), size = 1.5) + facet_wrap(facets = 'indicator', ncol = 4, scales = 'free_x') + theme_bw() + theme(legend.position = 'bottom') +
     scale_fill_viridis_d()+ scale_color_viridis_d() + guides(alpha = FALSE)
