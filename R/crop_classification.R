...
#' 
#' @export
calc_cropclass <- function(B_LU_BRP,B_BT_AK) {
  ...
  dt <- data.table(
    ...
    B_LU_BRP = B_LU_BRP,
    crop_sensitivity_p = NA_real_,
    crop_sensitivity_k = NA_real_,
    crop_sensitivity_s = NA_real_
  )
  ...
  nat = 'bos|rand|griend|natuur|houtwal|onbeteeld|onbekend|struweel|oever|riet|wandel|wilg|brandn|cultuur|poel|bomenrij'
  ...
  dt[grepl(grs,crop_name),crop_sensitivity_p := 'gras']
  dt[grepl(sms,crop_name),crop_sensitivity_p := 'mais'] 
  dt[grepl(cr0,crop_name),crop_sensitivity_p := 'class0']
  dt[grepl(cr1,crop_name),crop_sensitivity_p := 'class1']
  dt[grepl(cr2,crop_name),crop_sensitivity_p := 'class2']
  dt[grepl(cr3,crop_name),crop_sensitivity_p := 'class3']
  dt[grepl(cr4,crop_name),crop_sensitivity_p := 'class4']
  dt[grepl(nat,crop_name),crop_sensitivity_p := 'natuur'] 
  dt[grepl(bkw,crop_name),crop_sensitivity_p := 'bkw']
  dt[grepl('witlof|peen',crop_name) & grepl('zand|dalgrond',soiltype.n),crop_sensitivity_p := 'class0']
  
  # replace missing ones with low sensitivity
  dt[is.na(crop_sensitivity_p),crop_sensitivity_p :='class4']
  ...
  nat = 'bos|rand|griend|natuur|houtwal|onbeteeld|onbekend|struweel|oever|rietzoom|wandel|wilg|brandn|cultuur|poel|bomenrij'
  bkw = 'bomen|peren|bloem|rozen|facelia|hop|lupine|kers|bramen|heesters|elzen|pruimen|zonne|conifer|kruid|vaste|heg|boom|wijn|bloemkwek|heesters|appelen|buxus|bes'
  
  # distinghuish K sensitivity crop classes for grassland, nature and tree & bulbs
  dt[grepl(grs,crop_name), crop_sensitivity_k := 'gras']
  dt[grepl(nat,crop_name), crop_sensitivity_k := 'natuur'] 
  dt[grepl(bkw,crop_name), crop_sensitivity_k := 'bkw']
  
  # distinghuish K sensitivity arable crop classes on sandy soils
  soilsel <- 'zand|dekzand|dalgrond|veen'
  dt[grepl(cr1,crop_name) & grepl(soilsel,soiltype.n),crop_sensitivity_k := 'class1']
  dt[grepl(cr2,crop_name) & grepl(soilsel,soiltype.n),crop_sensitivity_k := 'class2']
  dt[grepl(cr3,crop_name) & grepl(soilsel,soiltype.n),crop_sensitivity_k := 'class3']
  dt[grepl(cr4,crop_name) & grepl(soilsel,soiltype.n),crop_sensitivity_k := 'class4']
  ...
  dt[grepl(cr1,crop_name) & soiltype == 'loess',crop_sensitivity_k := 'class1']
  dt[grepl(cr2,crop_name) & soiltype == 'loess',crop_sensitivity_k := 'class2']
  dt[grepl(cr3,crop_name) & soiltype == 'loess',crop_sensitivity_k := 'class3']
  ...
  dt[grepl(cr1,crop_name) & grepl('klei',soiltype.n), crop_sensitivity_k := 'class1']
  dt[grepl(cr2,crop_name) & grepl('klei',soiltype.n), crop_sensitivity_k := 'class2']
  dt[grepl(cr3,crop_name) & grepl('klei',soiltype.n), crop_sensitivity_k := 'class3']
  dt[grepl(cr4,crop_name) & grepl('klei',soiltype.n), crop_sensitivity_k := 'class4']
  dt[grepl(cr5,crop_name) & grepl('klei',soiltype.n), crop_sensitivity_k := 'class5']
  
  # replace missing ones with low sensitivity
  dt[is.na(crop_sensitivity_k), crop_sensitivity_k := 'class4']
  ...
  # distinghuish K sensitivity crop classes for grassland, nature and tree & bulbs
  dt[grepl(grs,crop_name), crop_sensitivity_s := 'gras']
  dt[grepl(nat,crop_name), crop_sensitivity_s := 'natuur'] 
  dt[grepl(bkw,crop_name), crop_sensitivity_s := 'bkw']
  
  # distinghuish S sensitivity arable crop classes 
  dt[grepl(cr1,crop_name), crop_sensitivity_s := 'class1']
  dt[grepl(cr2,crop_name), crop_sensitivity_s := 'class2']
  dt[grepl(cr3,crop_name), crop_sensitivity_s := 'class3']
  dt[grepl(cr4,crop_name), crop_sensitivity_s := 'class4']
  
  # replace missing ones with low sensitivity
  dt[is.na(crop_sensitivity_s), crop_sensitivity_s := 'class4']
