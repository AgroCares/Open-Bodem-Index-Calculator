...
#' @param advice (character) Optional parameter to select CEC index for soil aggregation or cation buffer capacity. Options: cationbuffer_index or aggregation_index
#'    
#' @import data.table
#' 
#' @export
calc_cec <- function(A_CEC_CO,A_K_CEC,A_CA_CEC,A_MG_CEC, A_OS_GV, B_BT_AK, advice) {
  ...
  id = cationbuffer_index = aggregation_index = NULL
  ...
  checkmate::assert_subset(advice, choices = c('cationbuffer_index','aggregation_index'), empty.ok = FALSE)
  ...
  dt <- data.table(
    ...
    B_BT_AK = B_BT_AK,
    A_OS_GV = A_OS_GV,
    cationbuffer_index = NA_real_,
    aggregation_index = NA_real_
    )
  
  # Calculate CEC index for soil cation buffer capacity
  dt[, cationbuffer_index := A_CEC_CO]
  
  # Calculate CEC index for soil aggregation on sandy soils (normalized to value beween 0-1)
  dt[grepl('zand|dal',B_BT_AK), aggregation_index := sqrt((A_CA_CEC - 80)^2 + (A_MG_CEC - 8)^2 + (A_K_CEC - 3.5)^2) / 125]
  
  # Calculate CEC index for soil aggregation on non-sandy soils (normalized to value beween 0-1)
  dt[!grepl('zand|dal',B_BT_AK), aggregation_index := sqrt((A_CA_CEC - 85)^2 + (A_MG_CEC - 8)^2 + (A_K_CEC - 3.5)^2) / 125]
  
  # Aggregate stability not relevant measure for peat soils
  dt[A_OS_GV > 25, aggregation_index := 0]
  
  # Restrict the value to be <= 1
  dt[, aggregation_index := pmin(1, aggregation_index)]
...
#' 
#' @export
ind_cec <- function(D_CEC,advice = 'cationbuffer_index') {
  
  # Check inputs
  if(advice == 'cationbuffer_index'){checkmate::assert_numeric(D_CEC, lower = 0, upper = 1000, any.missing = FALSE)} 
  if(advice == 'aggregation_index'){checkmate::assert_numeric(D_CEC, lower = 0, upper =  1, any.missing = FALSE)} 
  
  # Evaluate the CEC for agricultural production, given impact soil cation buffer capacity (Goselink & Van Erp, 1999)
  if(advice == 'cationbuffer_index'){value <- pmin(D_CEC * 0.01, 1)}
  
  # Evaluate the CEC for soil aggregation aspects (as used by Eurofins)
  if(advice == 'aggregation_index'){value <- 1 - D_CEC}
