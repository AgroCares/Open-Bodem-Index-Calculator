...
#' @param M_M4 (boolean) measure 4. is the soil ploughed (option: yes or no)
...
calc_management <- function(A_OS_GV,B_LU_BRP, B_BT_AK,B_GT,
  ...
  id = crop_code = crop_name = soiltype = soiltype.n = crop_n = NULL
  ...
  dt <- merge(dt, crops.obic[, list(crop_code, crop_n,crop_name)], by.x = "B_LU_BRP", by.y = "crop_code")
  ...
  dt.arable[grepl('mais',crop_name) & M_M15==TRUE, value := value + 1]
...
ind_management <- function(D_MAN,B_LU_BRP,B_BT_AK) {
  ...
  id = crop_code = soiltype = soiltype.n = crop_n = crop_name = NULL
