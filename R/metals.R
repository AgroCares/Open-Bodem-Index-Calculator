...
#' 
#' @export
calc_zinc_availability <- function(A_ZN_CC, B_LU_BRP, B_BT_AK) {
...
#' 
#' @export
ind_nitrogen <- function(D_CU) {
  
  # Check inputs
  checkmate::assert_numeric(D_CU, lower = -30, upper = 250, any.missing = FALSE)
  
  # Evaluate the nitrogen
  value <- OBIC::evaluate_parabolic(D_CU, x.top = 120)
...
#' 
#' @param D_ZN (numeric) The value of Cu-index  calculated by \code{\link{calc_zinc_availability}}
#' 
#' @export
ind_nitrogen <- function(D_ZN) {
  
  # Check inputs
  checkmate::assert_numeric(D_ZN, lower = -30, upper = 250, any.missing = FALSE)
  
  # Evaluate the nitrogen
  value <- OBIC::evaluate_parabolic(D_ZN, x.top = 120)
